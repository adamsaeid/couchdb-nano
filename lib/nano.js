...
module.exports = exports = nano = function dbScope(cfg) {
  ...
  function getCouchVersion(callback) {
    httpAgent(cfg.url, (err, resp) => {
      var version = JSON.parse(resp.body).version
  ...
  function docModule(dbName) {
    ...
    function queryDocs(queries, qs0, callback0) {
      const {opts, callback} = getCallback(qs0, callback0);

      return relax({
        db: dbName,
        path: '_all_docs/queries',
        method: 'POST',
        qs: opts,
        body: queries
      }, callback);
    ...
    function view(ddoc, viewName, meta, qs0, callback0) {
      ...
      if (qs1 && qs1.keys) {
        ...
        if (semver.satisfies(couchVersion, ">=2.2.0")) {
          viewPath = viewPath + '/queries';
        }
    ...
    function viewDocs(ddoc, viewName, qs, callback) {
      return new Promise(function (resolve) {
        getCouchVersion(() => {
          resolve(view(ddoc, viewName, {type: 'view'}, qs, callback))
        });
      })
    }
